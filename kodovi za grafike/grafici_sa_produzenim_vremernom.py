# -*- coding: utf-8 -*-
"""
Created on Wed May  7 23:32:34 2025
...
gefs_df = gefs_df[gefs_df['temp'].notna()]
meteostat_df_formated = format_data_frame_time(meteostat_df, gefs_df)
openmeteo_df_formated = format_data_frame_time(openmeteo_df, gefs_df)
visualcross_df_formated = format_data_frame_time(visualcross_df, gefs_df)
gfs_df_formated = format_data_frame_time(gfs_df, gefs_df)
gefs_df_formated = gefs_df
...
plt.scatter(visualcross_df_formated['time'], visualcross_df_formated['prcp'], label='Visualcross', color='purple')
...
x_ticks = np.linspace(0, len(gefs_df_formated['time']) - 1, 10, dtype=int)
tick_dates = gefs_df_formated['time'].iloc[x_ticks]
...
plt.plot(visualcross_df_formated['time'], visualcross_df_formated['temp'], label='Visualcross', color='purple')
plt.plot(gfs_df_formated['time'], gfs_df_formated['temp'], label='GFS', color='blue')
plt.plot(gefs_df_formated['time'], gefs_df_formated['temp'], label='GEFS', color='green')
